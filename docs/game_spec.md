...
* **무기 시스템 및 피격/데미지 충돌 판정:**
  * **직사 빔 (BEAM):**
    ...
    * **사격 형태:** 사격 시점의 함선 위치 $P_{beam}$에서 빔 사거리 및 발사 사각 클램핑 규칙이 적용되어 보정된 최종 $Target$ 좌표까지의 **선분(Segment)**.
    * **조준 클램핑 규칙 (사거리 및 사각 제한):**
      - **최대 사거리:** $600\text{px}$. 조준 벡터 거리가 $600$을 초과하면 최대 사거리 선상으로 타겟 위치를 클램핑합니다.
      - **최대 발사 사각:** 격발 시점(25틱)의 함선 기수 방향 기준 **좌우 $\pm 30^\circ$ ($\pm \pi / 6$ rad)**. 기수 방향과 조준 방향의 사이각 편차가 이를 초과하면 최대 허용 사각 한계선 방향으로 타겟 좌표를 클램핑 보정합니다.
    * **피격 판정:** 적 함선의 중심 $P_{enemy}$와 최종 보정된 빔 사격 선분 사이의 최단 거리가 **$25\text{px}$ 이하**일 때 피격 판정.
...
### 모바일 앱 출시 패키징 및 반응형 대응 사양
모바일 하이브리드 앱(iOS/Android) 래핑 및 모바일 화면 최적화를 위한 플랫폼별 연동 규격입니다.

1. **반응형 16:9 레터박스 뷰포트:**
   * 모바일 브라우저/웹뷰의 다양한 스케일 비율에 대응하기 위해, 게임 캔버스와 HUD의 논리 해상도를 $1280 \times 720$으로 고정하고 감싸는 `#app-container` 래퍼에 `CSS Transform: scale()` 동적 계산식을 적용합니다.
   * 모바일 터치 기기의 노치 디자인 대응을 위해 `<meta name="viewport">` 내에 `viewport-fit=cover` 속성을 설정하고 더블 탭 확대 줌 현상을 차단합니다.
2. **모바일 원터치 조작계:**
   * 모바일 기기에는 마우스 우클릭이 존재하지 않으므로, **"함선 본체를 잡고 드래그하면 추진 조타"**, **"함선 바깥 빈 화면의 임의 지점을 터치 탭하면 무기 조준"**이 가능하도록 포인터 업 탭 판정(유지 시간 250ms 이하 & 움직임 8px 미만) 보정을 사용합니다.
3. **Capacitor 패키징 빌드 가이드:**
   * 리포지토리의 `client` 디렉토리에 Capacitor 설정 파일 `capacitor.config.json` 이 제공됩니다.
   * **빌드 및 동기화 스크립트:**
     * `npm run mobile:init` : 클라이언트 빌드 후 Capacitor 연동 이니셜라이징을 기동합니다.
     * `npm run mobile:add-ios` / `npm run mobile:add-android` : 네이티브 iOS/Android 프로젝트 모듈을 생성합니다.
     * `npm run mobile:sync` : Vite 웹 코드 변경점을 네이티브 Xcode/Android Studio 프로젝트 에셋으로 자동 동기화합니다.
